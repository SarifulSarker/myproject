        ...
        <div class="card card-primary card-outline card-outline-tabs">
            ...
            <div class="card-body">
                ...
                <div class="tab-content" id="custom-tabs-four-tabContent">
                    <div class="tab-pane fade {{ request()->routeIs('student.individualpage') || request()->routeIs('student.searchbyindividual') ? 'show active' : '' }}" id="custom-tabs-four-home" role="tabpanel">
                        {{-- Content for "By Individual" tab --}}
                        <form id="searchForm" action="{{ route('student.searchbyindividual') }}" method="GET">
                            <div class="form-group">
                                <label for="searchInput">Search by Student ID:</label>
                                <input type="text"    class="form-control" id="searchindividual" name="searchindividual" placeholder="Search by student_id">
                            </div>
...
<script>
    ...
    document.getElementById('sendSMSBtn').addEventListener('click', function() {
        ...
        @if (isset($students) && !$students->isEmpty())
            var student = @json($students[0]); // Get the first student
...
<script>
    // Function to set the active tab based on the presence of search results
    function setActiveTab() {
        ...
        if (searchResultsExist) {
            tabLinks.forEach(function(link) {
                if (link.getAttribute('href') == "{{ route('student.searchbyindividual') }}") {
                    link.classList.add('active');
                }
            });
        } else {
            tabLinks.forEach(function(link) {
                if (link.getAttribute('href') == "{{ route('student.individualpage') }}") {
                    link.classList.add('active');
                }
            });
        }
